...
import { AlertTriangle, User, MapPin, Phone, CreditCard, Calendar, GraduationCap } from 'lucide-react';
...
const DoublonConfirmationModal: React.FC<DoublonConfirmationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className={`${cardClasses} rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto border`}>
        <div className="p-6">
          {/* En-tête */}
          ...
          <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg">
            <p className="text-sm text-orange-800 dark:text-orange-200">
              <strong>Attention :</strong> Nous avons trouvé {doublons.length} personne(s) avec le même nom, prénom, lieu de naissance et informations de contact. 
              Voulez-vous continuer l'ajout ou annuler ?
            </p>
          </div>

          {/* Liste des doublons */}
          <div className="mb-6">
            <h3 className="text-lg font-semibold mb-4">Personnes similaires trouvées :</h3>
            <div className="space-y-4">
              {doublons.map((doublon, index) => (
                <div key={doublon.id} className={`${cardClasses} p-4 rounded-lg border`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-600 dark:text-gray-300">
                            Né(e) le {new Date(doublon.date_naissance).toLocaleDateString('fr-FR')}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            ...
                            {doublon.statut.charAt(0).toUpperCase() + doublon.statut.slice(1)}
                          </span>
                        </div>
                      </div>
                    </div>
