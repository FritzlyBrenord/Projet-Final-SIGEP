...
interface ProtectedRouteProps {
  ...
  showLoader?: boolean;
  requireUuidValidation?: boolean; // Nouvelle prop pour contrôler la validation UUID
}
...
const extractUuidFromPath = (pathname: string): string | null => {
  // Pour une route comme /SIGEP-Tableau-De-Bord/ca905dca-547c-4a41-9af7-f6ef15d80d90
  const segments = pathname.split("/");

  // Chercher le segment qui contient "SIGEP-Tableau-De-Bord"
  const dashboardIndex = segments.findIndex((segment) =>
    segment.includes("SIGEP-Tableau-De-Bord")
  );

  // L'UUID devrait être le segment suivant
  ...
  // Alternative : chercher directement un UUID dans l'URL
...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({
  children,
  showLoader = true,
  requireUuidValidation = true, // Par défaut, la validation UUID est requise
}) => {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const params = useParams(); // Nouveau hook pour les paramètres de route
  ...
  useEffect(() => {
    const checkAccess = () => {
      ...
      // === VÉRIFICATION UUID OBLIGATOIRE ===
      if (requireUuidValidation) {
        // Vérifier que l'UUID existe dans localStorage
        if (!uuid) {
        ...
        // Vérifier que l'UUID a un format valide
        if (!isValidUuid(uuid)) {
        ...
        // Extraire l'UUID de l'URL (route dynamique)
        const urlUuid = extractUuidFromPath(pathname);

        // Alternative avec useParams si la route est définie comme [uuid]
        const paramUuid = params?.uuid as string;

        // Utiliser l'UUID trouvé (priorité au paramètre de route)
        const routeUuid = paramUuid || urlUuid;

        if (routeUuid) {
          // Comparaison stricte des UUID
          if (routeUuid !== uuid) {
          ...
          // Si aucun UUID trouvé dans l'URL et que la validation est requise
      ...
      // Si l'utilisateur est authentifié, accorder l'accès immédiatement
      if (currentSession.isAuthenticated) {
        setAccessGranted(true);
        setIsChecking(false);
        setWaitingForSession(false);

        // Nettoyer les timeouts si l'authentification réussit
        if (timeoutRef.current) {
          clearTimeout(timeoutRef.current);
      ...
      // Si pas d'authentification et qu'on n'attend pas encore
      if (!currentSession.isAuthenticated && !waitingForSession) {
        ...
        // Timeout de grâce - attendre avant de rediriger
        timeoutRef.current = setTimeout(() => {
          // Vérifier une dernière fois si la session est maintenant disponible
          if (!currentSession.isAuthenticated) {
...
export const useRouteProtection = () => {
  ...
  // Fonction de déconnexion sans modal
