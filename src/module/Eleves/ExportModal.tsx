...
import { X, Plus, Trash2, Download, Building, MapPin, Phone, FileText, Table, Printer } from "lucide-react";
...
const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const [availableColumns, setAvailableColumns] = useState<ExportColumn[]>([
    { id: "code", label: "Code", key: "code", isSelected: true, isCustom: false },
    { id: "nom", label: "Nom", key: "nom", isSelected: true, isCustom: false },
    { id: "prenom", label: "Prénom", key: "prenom", isSelected: true, isCustom: false },
    { id: "sexe", label: "Sexe", key: "sexe", isSelected: false, isCustom: false },
    { id: "dateNaissance", label: "Date de naissance", key: "dateNaissance", isSelected: false, isCustom: false },
    { id: "lieuNaissance", label: "Lieu de naissance", key: "lieuNaissance", isSelected: false, isCustom: false },
    { id: "adresseActuelle", label: "Adresse actuelle", key: "adresseActuelle", isSelected: false, isCustom: false },
    { id: "telephoneParents", label: "Téléphone parents", key: "telephoneParents", isSelected: false, isCustom: false },
    { id: "nifParents", label: "NIF parents", key: "nifParents", isSelected: false, isCustom: false },
    { id: "adresseParents", label: "Adresse parents", key: "adresseParents", isSelected: false, isCustom: false },
    { id: "classesDemandee", label: "Classe", key: "classesDemandee", isSelected: true, isCustom: false },
    { id: "salle", label: "Salle", key: "salle", isSelected: true, isCustom: false },
    { id: "moyenneGenerale", label: "Moyenne générale", key: "moyenneGenerale", isSelected: false, isCustom: false },
    { id: "etablissementPrecedent", label: "Établissement précédent", key: "etablissementPrecedent", isSelected: false, isCustom: false },
    { id: "status", label: "Statut", key: "status", isSelected: false, isCustom: false },
    { id: "dateInscription", label: "Date d'inscription", key: "dateInscription", isSelected: false, isCustom: false },
    { id: "observations", label: "Observations", key: "observations", isSelected: false, isCustom: false },
  ]);
  ...
  const getFilteredStudents = () => {
    ...
    if (selectedClass) {
      filtered = filtered.filter(student => student.classesDemandee === selectedClass);
    }
...
<body>
    <div class="header">
        ...
        <p><strong>Adresse:</strong> ${etablissementInfo.adresse}</p>
        <p><strong>Téléphone:</strong> ${etablissementInfo.telephone}</p>
    </div>
    ...
    <div class="info">
        ...
        ${selectedSalle ? `<p><strong>Salle:</strong> ${selectedSalle}</p>` : ''}
        ${selectedStatus ? `<p><strong>Statut:</strong> ${selectedStatus}</p>` : ''}
    </div>
                ...
                <tr>
                    ${selectedColumns.map(col => {
                        ...
                        if (col.key === 'sexe') {
                            ...
                            value = `<span class="${statusClass}">${String(value).charAt(0).toUpperCase() + String(value).slice(1)}</span>`;
    ...
    <div class="footer">
        <p>Document généré le ${currentDate} par le système SIGEP</p>
    </div>
</body>
</html>`;
  ...
  const downloadHTML = () => {
    ...
    link.href = url;
    link.download = `liste_eleves_${new Date().toISOString().split('T')[0]}.html`;
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Date d'export: ${new Date().toLocaleDateString('fr-FR')}`, 14, 52);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(
        `Page ${i} sur ${pageCount} - Document généré le ${new Date().toLocaleDateString('fr-FR')} par le système SIGEP`,
        14,
  ...
  return (
    <div className="fixed inset-0 bg-black/60 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className={`${baseClasses} rounded-lg max-w-6xl w-full max-h-[90vh] overflow-y-auto border`}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold">Exporter la liste des élèves</h2>
            <button
              onClick={onClose}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={etablissementInfo.nom}
                  onChange={(e) => setEtablissementInfo(prev => ({ ...prev, nom: e.target.value }))}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 ${inputClasses}`}
                  value={etablissementInfo.adresse}
                  onChange={(e) => setEtablissementInfo(prev => ({ ...prev, adresse: e.target.value }))}
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 ${inputClasses}`}
                  value={etablissementInfo.telephone}
                  onChange={(e) => setEtablissementInfo(prev => ({ ...prev, telephone: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {selectedClass && sallesByClass[selectedClass]
                    ...
                    : Object.values(sallesByClass).flat().map(salle => (
                        <option key={salle} value={salle}>{salle}</option>
                      ))}
                </select>
              </div>
              ...
              <div className={`p-3 rounded-lg ${
                ...
                <p className="text-sm">
                  <strong>Résultat:</strong> {getFilteredStudents().length} élève(s) trouvé(s)
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            {customColumns.length > 0 && (
              <div className="mt-4">
                <h4 className="text-md font-medium mb-2">Colonnes personnalisées</h4>
                    ...
                    <div key={column.id} className="flex items-center gap-2">
                      <input
                        ...
                        onChange={() => {
                          setCustomColumns(prev => 
                            prev.map(col => 
                              col.id === column.id ? { ...col, isSelected: !col.isSelected } : col
                            )
                          );
                        }}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      ...
                      <input
                        type="text"
                        value={column.label}
                        onChange={(e) => updateCustomColumnLabel(column.id, e.target.value)}
